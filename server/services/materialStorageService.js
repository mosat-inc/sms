...
const normalizeMaterialRecord = async (record, { includeAccessUrls = true } = {}) => {
    ...
    const normalized = {
        ...
        sizeBytes: record.size_bytes || record.file_size || 0,
        uploadedBy: record.uploaded_by || record.teacher_id,
        visibility: record.visibility_scope || (record.is_public ? 'public' : 'private'),
        accessRole: record.access_role || 'teacher',
        category: record.category || 'teaching_material',
        createdAt: record.created_at,
        updatedAt: record.updated_at,
        downloadCount: record.download_count || 0,
        tags,
        uploadStatus: record.upload_status || 'ready',
        storageProvider: record.storage_provider || 'local',
        fileAvailable: Boolean(record.object_key) && (record.upload_status || 'ready') === 'ready',
